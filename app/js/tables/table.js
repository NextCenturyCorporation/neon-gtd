 ...
 * @private
 */
tables.createColumns = function(data, ignoreFields, headerElements) {
    ...
    data.forEach(function(row) {
        ...
        for(var i = 0; i < keys.length; ++i) {
            var key = keys[i];
            if(ignoreFields && ignoreFields.length && ignoreFields.indexOf(key) >= 0) {
                continue;
            }
    ...
    Object.keys(columnNameToLongestText).forEach(function(name) {
        element.html(columnNameToLongestText[name]);
        if(name === columnNameToLongestText[name]) {
            element.append(sortElement).append(resizeElement);
            if(headerElements) {
                for(var i = 0; i < headerElements.length; ++i) {
                    element.append($(headerElements[i]));
                }
            }
        }
        columns.push({
            ...
            width: Math.min(element.outerWidth() + 5, 800)
        });
    });
